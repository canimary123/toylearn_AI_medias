...
````
이전에 요청하신 **GitHub 마크다운 형식**의 텍스트와 이전에 누락되었던 내용들을 모두 확인하고 검증하여, 나머지 프롬프트 패턴들을 깔끔하게 정리했습니다.

-----

## **글 교정의 슈퍼파워, 전문 편집자 고용하기 (지속 모드)**

### **5. 지속적인 교정 모드 (Continuous Correction Mode)**
...
이 대화에서는 지속적인 교정 모드로 동작합니다. 사용자가 어떤 내용의 새 텍스트를 입력하든, 그것을 \*\*`[원본]`\*\*으로 간주하고 즉시 교정을 진행합니다. 사용자가 특별히 "교정 모드 종료"라고 입력하기 전까지는 모든 입력을 교정 대상으로 간주합니다.
  ...
  * **교정 결과 활용:** 출력된 여러 교정본 중에서 마음에 드는 것을 고르고, 때로는 여러 버전에서 마음에 드는 일부만 선택하여 최종 버전을 만드는 것이 좋은 방법입니다.
  * **핵심 목표:** 사용자가 글의 품질을 집중적으로 높여 **글 전체를 이전보다 더 좋은 글**로 고치는 것이 핵심입니다. 단순한 문장 부호 교정이나 일부 단어만 고치는 것보다 훨씬 효과적입니다.
  * **주의사항:** 너무 긴 글을 한 번에 교정하려고 하면 모델이 전체 맥락을 놓치거나 일관된 교정을 하지 못할 수도 있습니다.
...
## **[보너스] AI 프로이트에게 내 꿈을 해석 받아보면 어떨까?**
